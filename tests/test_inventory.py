...
from products import add_product, search_products
from stocks import (
    add_stock, available_quantity, calculate_purchase_quantity,
    consume_stock, expiring_stocks, get_expiry_status, get_stock_status,
...
def test_fractional_consumption_does_not_leave_phantom_stock(inventory):
    ...
    assert stocks == []
    assert calculate_purchase_quantity(0.1, 0.3) == 0.2
...
def test_pr1_expiry_boundaries(days, expected):
    assert get_expiry_status(TODAY + timedelta(days=days), TODAY).startswith(
        expected
    )


def test_pr1_stock_status_and_no_unnecessary_purchase():
    assert get_stock_status(0, 5) == "Запас отсутствует"
    assert get_stock_status(2, 5) == "Запас нужно пополнить"
    assert get_stock_status(5, 5) == "Запаса достаточно"
    assert calculate_purchase_quantity(7, 5) == 0
